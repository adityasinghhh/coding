...
//		printf("Query: %d %d %d\n", x, y, z);
		...
		for(j=size-1; j>=0; j--)
		{
			...
			if(tree[path[j].first][path[j].second]->S[size1-1][0] < x)
				continue;

			int l = 0, e = size-1, mid;

			while(l <= e)
			{
				mid = (l+e)/2;

				if(tree[path[j].first][path[j].second]->S[mid][0] < x && tree[path[j].first][path[j].second]->S[mid+1][0] >= x)
					break;
				else if(tree[path[j].first][path[j].second]->S[mid][0] < x)
					l = mid+1;
				else
					e = mid-1;
			}
