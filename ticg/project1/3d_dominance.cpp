...
//		printf("\n");
		...
		for(j=size-1; j>=0; j--)
		{
			...
			size1 = tree[path[j].first+1][right]->S.size();

			l = 0;
			e = size1-1;
			
			if(tree[path[j].first+1][right]->S[e][0] < x)
				continue;
			...
			while(l < e)
			{
				...
				if(tree[path[j].first+1][right]->S[mid][0] < x && tree[path[j].first+1][right]->S[mid+1][0] >= x)
					break;

				else if(tree[path[j].first+1][right]->S[mid][0] < x)
					l = mid;
				else
					e = mid;
			}
